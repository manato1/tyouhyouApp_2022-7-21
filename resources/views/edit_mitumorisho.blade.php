...
<div class="create-box" id="form">
    <form action="/mitumorisho_pdf" method="POST" target="blank">
        @csrf
        <div class="box">
        @foreach($quotation as $quotation)
        <label for="from">From</label><input type="text" name="from" id="from" value="{{ $quotation -> from }}" v-model="from" required> 
        <p id="subtext1">スペースで改行</p>
        <label for="for">For</label><input type="text" name="for_for" id="for" value="{{ $quotation -> for }}" v-model="For" required>
        <p id="subtext1">スペースで改行</p>
        <label for="date">見積日</label><input type="date" name="date" id="date" value="{{ $quotation -> date }}" v-model="date" required>
        <label for="deadline">有効期限</label><input type="date" name="deadline" id="deadline" value="{{ $quotation -> deadline }}" v-model="deadline" required>
        <label for="subject">件名</label><input type="text" name="subject" id="subject" value="{{ $quotation -> subject }}" v-model="subject">
        <label for="number">見積No</label><input type="number" name="number" id="number" value="{{ $quotation -> quote_no }}" v-model="number">
        ...
        <div class="summary">
            ...
            <div class="content">品目</div><div class="unit_price">単価</div><div class="quantity">数量</div><div class="rate">税率</div>
                    ...
                    <input type="number" name="quantity_{{ $i }}" class="quantity" id="quantity_{{ $i }}" value="{{ $quotation_item -> quantity }}" v-model="quantity_{{ $i }}">
                    <input type="number" name="rate_{{ $i }}" class="rate" id="rate_{{ $i }}" value="{{ $quotation_item -> rate }}" v-model="rate_{{ $i }}">
                    <?php $i ++;?>
                    @endforeach
                    @for($i;$i<=18;$i++)
        ...
        <div class="box">
            <label for="remarks">備考</label><textarea name="remarks" id="remarks" cols="30" rows="10" v-model="remarks">{{ $quotation -> remarks }}</textarea>
        </div>
        @endforeach
...
<div class="hyoutyou">
        ...
        <div class="box-1">
            ...
            <div class="box-right">
                <div class="number">見積No.@{{ Number }}</div> 
                <div class="date">見積日 @{{ Date }}</div>
